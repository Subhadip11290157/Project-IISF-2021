...
while True:
    ...
    if(len(landmark_list) != 0):
        ...
        if (my_fingers[1] and my_fingers[2]):
            ...
            k_shiftt = 110
            if (y1 < 125):
                if(200 < x1 < 340 + k_shiftt):
                    default_overlay = overlay_image[0]
                    draw_color = (150, 12, 8)
                elif (340 + k_shiftt < x1 < 500 + k_shiftt):
                    default_overlay = overlay_image[1]
                    draw_color = (16, 227, 213)
                elif (500 + k_shiftt < x1 < 640 + k_shiftt):
                    default_overlay = overlay_image[2]
                    draw_color = (247, 22, 240)
                elif (640 + k_shiftt < x1 < 780 + k_shiftt):
                    default_overlay = overlay_image[3]
                    draw_color = (34, 219, 29)
                elif (1100 + k_shiftt-80 < x1 < 1280):
                    default_overlay = overlay_image[4]
                    draw_color = (0, 0, 0)

            cv2.putText(frame, 'Color Selector Mode', (900, 680), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(
        ...
        if (my_fingers[1] and not my_fingers[2]):
            ...
            if draw_color == (0, 0, 0):
                cv2.line(frame, (xp, yp), (x1, y1), color=draw_color,
                         thickness=eraser_thickness)
                cv2.line(image_canvas, (xp, yp), (x1, y1),
                         color=draw_color, thickness=eraser_thickness)

            else:
                cv2.line(frame, (xp, yp), (x1, y1),
                         color=draw_color, thickness=brush_thickness)
                cv2.line(image_canvas, (xp, yp), (x1, y1),
                         color=draw_color, thickness=brush_thickness)
    ...
    cv2.putText(frame, 'Client FPS:' + str(int(fps)), (10, 670),
